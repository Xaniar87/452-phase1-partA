...
int P3_Startup(void *notused) 
{
	...
	printf("P3 pid changed: %d\n", pid);
	assert(result == 0);
	result = Sys_Spawn("P5_Startup", P5_Startup, NULL, 4 *  USLOSS_MIN_STACK, 4, &pid);
        printf("P4 pid changed: %d\n", pid);
	assert(result == 0);
	result = Sys_Spawn("P6_Startup", P6_Startup, NULL, 4 *  USLOSS_MIN_STACK, 5, &pid);
        printf("P4 pid changed: %d\n", pid);
	assert(result == 0);
	Sys_DumpProcesses();
...
int P5_Startup(void *notused)
{
	int pid;
 	...
 	int result = Sys_Spawn("P6_Startup", P6_Startup, NULL, 4 *  USLOSS_MIN_STACK, 5, &pid);
